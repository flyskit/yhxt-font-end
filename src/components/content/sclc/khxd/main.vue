  ...
  div.khxd
    Form(ref="data" :model="data" :rules="rules" :label-width="80" inline)
      FormItem(label="编号" prop="num" )
        Input.input-width-in-form(v-model="data.num" placeholder="输入编号")
      FormItem(label="创建日期" prop="createTime")
        DatePicker.input-width-in-form(v-model="data.createTime" type="date" placeholder="输入订单日期")
      FormItem(label="客户" prop="customerName")
        Input.input-width-in-form(v-model="data.customerName" placeholder="输入客户名称")
      FormItem(label="地址" prop="customerAddr")
        Input.input-width-in-form(v-model="data.customerAddr" placeholder="输入客户地址")
      FormItem(label="出货日期" prop="deliverTime")
        DatePicker.input-width-in-form(v-model="data.deliverTime" type="date" placeholder="输入订单日期")
      FormItem(label="下单类型" prop="doorType")
        Cascader.input-width-in-form(v-model="data.doorType" :data="doorType" @on-change="getForm" trigger="hover")
      pre
