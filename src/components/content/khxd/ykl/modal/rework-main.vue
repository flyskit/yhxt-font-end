...
import { GET_REWORK_DATA, GETTER_REWORK_DATA, GETTER_TOTAL_ELEMENT } from '@store/common/khxd/ykl/xjbd/index';
...
export default {
  ...
  computed: {
    ...mapGetters({
      totalElement: 'commonKhxdYklXjbdIndex/' + GETTER_TOTAL_ELEMENT,
      mapOrderDetailList: 'commonKhxdYklXjbdIndex/' + GETTER_REWORK_DATA
    })
  },
  methods: {
    /** 显示 */
    ...
    /** 查询 */
    select() {
      this.$store.dispatch('commonKhxdYklXjbdIndex/' + GET_REWORK_DATA, this.orderQueryCond).then(() => {
    ...
    getSizeDetail(row) {
      this.$store.dispatch('commonKhxdYklJryxdIndex/' + GET_DATA_BY_BH, row.ddbh).then(res => {
        if (res.data.status !== 200) {
          ...
          this.acrylicDetail = res.data.map.data.acrylicDetail;
    ...
    createRework() {
      if (this.orderSize.length === 0) {
        ...
        this.$refs.reworkDataModal.show(this.acrylicDetail, this.orderDetail, this.orderSize);
      }
    },
    /** 关闭 */
    ok() {
