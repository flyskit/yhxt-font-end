  ...
  div.khxd-jgm-xjbd
    ...
    Form(ref="orderDetail" :model="orderDetail" :label-width="80" inline)
      ...
      FormItem(label="电话" prop="dh")
        Input.input-small(v-model="orderDetail.dh" autofocus=true)
      FormItem(label="拉手" prop="ls")
        AutoComplete.input-small(v-model="orderDetail.ls" @on-search="getHandleList")
      ...
      FormItem(label="备注" prop="bz")
        Input.input-small(v-model="orderDetail.bz")
...
export default {
  ...
  methods: {
    ...
    /** 导入拉手信息 */
    handleImport(orderDetail) {
      this.handleInfo.forEach((e) => {
        if (e.label === orderDetail.ls) {
          orderDetail.dj = e.dj;
    ...
    submitData(sizeDetail, orderStatus) {
      ...
      sizeDetail.forEach((e) => {
        this.defineProperty(e, '_index', '_orderDetailKey');
      });
      this.defineProperty(this.orderDetail, '_index', '_orderDetailKey');
      this.addData(sizeDetail);
    },
    /** 提交数据 */
    addData(sizeDetail) {
      this.$store.dispatch('commonKhxdJgmXjbdIndex/' + ADD_DATA, {...this.orderDetail, cupboardDoorSizes: sizeDetail}).then(res => {
