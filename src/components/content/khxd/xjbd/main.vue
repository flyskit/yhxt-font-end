  ...
  div.xjbd
    Form(ref="data" :model="data" :label-width="80" inline)
      FormItem(label="日期" prop="createTime")
        DatePicker.input-width-in-form(v-model="data.createTime" type="date" placeholder="输入下单日期")
      ...
      FormItem(label="电话" prop="customerTel")
        Input.input-width-in-form(v-model="data.customerTel" placeholder="输入联系方式")
      FormItem(label="下单" prop="orderType")
        Select.input-width-in-form(v-model="data.orderType" @on-change="getForm" clearable filterable remote)
